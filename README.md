...
# Bind landability to the exact target OID and conservative effective paths.
# Refresh this after the target advances and before changing Git.
...
# After an external Git operation makes the commit reachable from the exact
# target-scope preimage (fast-forward, or a merge with that preimage first):
